-- ORDER-4 · Checkout: payment_method + RPC seguro para crear pedidos (invitado o autenticado)
-- Idempotente donde aplica.

-- ---------------------------------------------------------------------------
-- payment_method en orders (preferencia elegida en checkout; sin cobro aún)
-- ---------------------------------------------------------------------------
alter table public.orders
  add column if not exists payment_method text;

update public.orders
set payment_method = 'pay_in_store'
where payment_method is null;

alter table public.orders
  alter column payment_method set default 'pay_in_store';
...
begin
  if not exists (
    ...
    alter table public.orders
      ...
      check (payment_method in ('mercado_pago', 'bank_transfer', 'pay_in_store'));
  end if;
end $$;

-- ---------------------------------------------------------------------------
-- create_order_from_checkout · SECURITY DEFINER
--
-- Inserta orders + order_items + customer_addresses (opcional) + order_status_history
-- en una transacción. Valida campos mínimos. Genera order_number RS-YYYY-NNNNNN
-- con retry ante colisión unique.
--
-- Llamable por anon y authenticated (checkout público). La validación de negocio
-- (productos publicados, stock, precios) ocurre en Server Action antes de invocar.
-- ---------------------------------------------------------------------------
...
grant execute on function public.create_order_from_checkout(jsonb) to anon, authenticated;
